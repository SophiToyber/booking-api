...
class BookingControllerIntegrationTest {
  ...
  void pay_PendingBooking_ShouldReturnPaid() throws Exception {
    // Given - create booking
    BookingCreateRequest createRequest = new BookingCreateRequest(
        testUnit.getId(),
        testUser.getId(),
    ...
    // When & Then
    mockMvc.perform(put("/api/bookings/{id}/pay", bookingId))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(bookingId))
  ...
  @Test
  @DisplayName("Should return 400 when trying to pay for cancelled booking")
  void pay_CancelledBooking_ShouldReturnBadRequest() throws Exception {
  ...
  void fullBookingLifecycle_CreatePayCancel_ShouldWorkCorrectly() throws Exception {
    // CREATE
    BookingCreateRequest createRequest = new BookingCreateRequest(
        testUnit.getId(),
        testUser.getId(),
